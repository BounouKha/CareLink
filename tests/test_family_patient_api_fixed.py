...
def test_family_patient_api():
    ...
    # Step 1: Login as family patient to get access token
    login_url = "http://localhost:8000/account/login/"
    admin_login_data = {
        "email": "fpatient1@carelink.be",  # Use family patient account
        "password": "Pugu8874@"
    }
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Family patient login failed: {response.status_code}")
            print(response.text)
            return
            
        login_response = response.json()
        access_token = login_response['access']
        print("✅ Family patient login successful")
        ...
        # Step 4: Test with a known family patient user
