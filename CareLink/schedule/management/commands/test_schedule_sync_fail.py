"""
Django management command to test schedule data synchron            for schedule in schedules:
                ...
                if schedule.provider and schedule.provider.user:
                    ...
                    provider_info = provider_name if provider_name else user.username
                else:
                    provider_info = "No Provider"
                timeslot_count = schedule.timeslots.count()        
provider_info = f"{schedule.provider.user.first_name} {schedule.provider.user.last_name}".strip() or schedule.provider.user.username if schedule.provider else "No Provider"ation.

Usage: python manage.py test_schedule_sync_fail
"""
    ...
    def handle(self, *args, **options):
        ...
        if schedules.exists():
            ...
            for schedule in schedules:
                patient_info = f"{schedule.patient}" if schedule.patient else "Blocked Time"
                provider_info = f"{schedule.provider.user.firstname} {schedule.provider.user.lastname}".strip() or schedule.provider.user.username if schedule.provider else "No Provider"
                timeslot_count = schedule.time_slots.count()
                
                self.stdout.write(f'Date: {schedule.date}')
                self.stdout.write(f'  Patient: {patient_info}')
                self.stdout.write(f'  Provider: {provider_info}')
        ...
        empty_schedules = schedules.filter(time_slots__isnull=True)
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f'Could not check service demands: {e}'))
